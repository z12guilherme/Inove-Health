...
export function generateTissXml(lote: LoteTiss, atendimentos: AtendimentoData[]): string {
  ...
  const processItems = (atd: AtendimentoData) => {
    ...
    (atd.procedimentos || []).forEach(p => {
      ...
      const unitario = Number(p.valor) || 0;
      const qtd = Number(p.qtd) || 1;
      const valorTotal = unitario * qtd;
      ...
      if (isDespesa(tab)) {
        ...
        else if (codDesp === '08') sumOPME += valorTotal;
      } else {
    ...
    const sumTotalGeral = parseFloat((sumProcedimentos + sumDiarias + sumTaxas + sumMateriais + sumMedicamentos + sumGases + sumOPME).toFixed(2));

    return { procs, desps, sumProcedimentos, sumDiarias, sumTaxas, sumMateriais, sumMedicamentos, sumGases, sumOPME, sumTotalGeral };
  };

  const buildProcedimentosXml = (procs: any[], atd: AtendimentoData, isInternacao: boolean) => {
    if (procs.length === 0) return '';
    const xml = procs.map(p => {
      const vUnit = Number(p.valor) || 0;
      const vTotal = vUnit * (Number(p.qtd) || 1);
      return `<ans:procedimentoExecutado><ans:sequencialItem>${p.seq}</ans:sequencialItem><ans:dataExecucao>${atd.data.split('T')[0]}</ans:dataExecucao><ans:procedimento><ans:codigoTabela>${(p.codigo_tabela || '22').split(' - ')[0]}</ans:codigoTabela><ans:codigoProcedimento>${p.codigo}</ans:codigoProcedimento><ans:descricaoProcedimento>${(p.nome || '').trim()}</ans:descricaoProcedimento></ans:procedimento><ans:quantidadeExecutada>${p.qtd || 1}</ans:quantidadeExecutada><ans:viaAcesso>1</ans:viaAcesso>${isInternacao ? '<ans:tecnicaUtilizada>1</ans:tecnicaUtilizada>' : ''}<ans:reducaoAcrescimo>1.00</ans:reducaoAcrescimo><ans:valorUnitario>${vUnit.toFixed(2)}</ans:valorUnitario><ans:valorTotal>${vTotal.toFixed(2)}</ans:valorTotal>${isInternacao ? `<ans:identEquipe><ans:identificacaoEquipe><ans:grauPart>00</ans:grauPart><ans:codProfissional><ans:cpfContratado>83365982272</ans:cpfContratado></ans:codProfissional><ans:nomeProf>VALDEMILSON ALVES</ans:nomeProf><ans:conselho>06</ans:conselho><ans:numeroConselhoProfissional>8696</ans:numeroConselhoProfissional><ans:UF>26</ans:UF><ans:CBOS>225250</ans:CBOS></ans:identificacaoEquipe></ans:identEquipe>` : ''}</ans:procedimentoExecutado>`;
    }).join('\n');
  ...
  const buildDespesasXml = (desps: any[], atd: AtendimentoData) => {
    ...
    const xml = desps.map(p => {
      const vUnit = Number(p.valor) || 0;
      const vTotal = vUnit * (Number(p.qtd) || 1);
      return `<ans:despesa><ans:sequencialItem>${p.seq}</ans:sequencialItem><ans:codigoDespesa>${getCodigoDespesa(p.codigo_tabela || '19')}</ans:codigoDespesa><ans:servicosExecutados><ans:dataExecucao>${atd.data.split('T')[0]}</ans:dataExecucao><ans:horaInicial>08:00:00</ans:horaInicial><ans:horaFinal>08:00:00</ans:horaFinal><ans:codigoTabela>${(p.codigo_tabela || '19').split(' - ')[0]}</ans:codigoTabela><ans:codigoProcedimento>${p.codigo}</ans:codigoProcedimento><ans:quantidadeExecutada>${(Number(p.qtd) || 1).toFixed(1)}</ans:quantidadeExecutada><ans:unidadeMedida>001</ans:unidadeMedida><ans:reducaoAcrescimo>1.00</ans:reducaoAcrescimo><ans:valorUnitario>${vUnit.toFixed(2)}</ans:valorUnitario><ans:valorTotal>${vTotal.toFixed(2)}</ans:valorTotal><ans:descricaoProcedimento>${(p.nome || '').trim()}</ans:descricaoProcedimento></ans:servicosExecutados></ans:despesa>`;
    }).join('\n');
  ...
  const buildGuiaConsulta = (atd: AtendimentoData) => {
    ...
    const procValor = parseFloat((atd.procedimentos?.[0]?.valor || 150.00).toFixed(2));
...
<ans:mensagemTISS xmlns:ans="http://www.ans.gov.br/padroes/tiss/schemas">
  ...
  // 2. Extrair EXATAMENTE o conteúdo entre as tags para o Hash
  ...
  return xmlString.replace('PLACEHOLDER_HASH', hashVal);
}

/**
 * Dispara o download do XML garantindo o encoding ISO-8859-1.
 * Essencial para que o validador da operadora não rejeite o arquivo por causa do Hash.
 */
export function downloadTissXml(xmlString: string, numeroLote: string) {
  const filename = `LOTE_${numeroLote}_${new Date().getTime()}.xml`;

  // Converte a string (UTF-16) para um array de bytes ISO-8859-1 (Uint8Array)
  const bytes = new Uint8Array(xmlString.length);
  for (let i = 0; i < xmlString.length; i++) {
    bytes[i] = xmlString.charCodeAt(i) & 0xff;
  }
