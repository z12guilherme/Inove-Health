...
// Seed data para autocompletes locais
const SEED_CIDS = [
...
const SEED_PROCEDIMENTOS = [
  { codigo: '10101012', nome: 'Consulta em consultório (médico assistente)', valor: 80.00 },
  ...
  { codigo: '40304321', nome: 'Urina rotina (EAS)', valor: 18.00 },
  { codigo: '40309999', nome: 'Dosagem de Creatinina', valor: 12.00 }
];

const SEED_CONSULTORIOS = [
  'Consultório 01 - Térreo',
  'Consultório 02 - Térreo',
  'Consultório 03 - 1º Andar',
  'Consultório 04 - 1º Andar',
  'Consultório 05 - Pediatria'
];
...
export function NovaConsultaEletiva() {
  ...
  const [consultorio, setConsultorio] = useState(SEED_CONSULTORIOS[0]);
  ...
  // Ações de inserção de procedimento
  const handleAddProcedimento = () => {
    ...
    const novo: ProcedimentoAdicionado = {
      ...
      especialidade: selectedMedico?.especialidade || 'Clínica Geral',
      profissional: selectedMedico?.nome || 'Médico Executante'
    };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        categoria,
        triagem_concluida: true, // Eletivas geralmente não passam pela triagem de Manchester
        dados_atendimento: {
          consultorio,
      ...
      toast.success('Consulta Eletiva registrada com sucesso!');
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Nova Consulta Eletiva</h1>
          <p className="text-muted-foreground mt-1">Fluxo de Recepção / Agendamento e Consulta ambulatorial.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="glass rounded-2xl p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            <label className="block text-sm font-medium mb-2 text-foreground">Regime de Faturamento / Categoria</label>
        ...
        <div className="glass rounded-2xl p-6 space-y-6">
          <div className="flex justify-between items-center border-b border-border/50 pb-4">
            <h2 className="text-xl font-semibold text-foreground flex items-center gap-2">
              <Stethoscope className="w-5 h-5 text-primary" /> Dados do Beneficiário
            </h2>
            <button
              type="button"
              onClick={() => {
        ...
        <div className="glass rounded-2xl p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <label className="block text-sm font-medium mb-2 text-foreground">Consultório</label>
              <select
                value={consultorio}
                onChange={e => setConsultorio(e.target.value)}
                className="w-full h-12 px-4 rounded-xl bg-background border border-border focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all text-foreground"
              >
                {SEED_CONSULTORIOS.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            ...
            <div className="relative" ref={cidRef}>
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Pesquise por CID ou nome..."
                  value={cidSearch}
                  onChange={e => {
                    const val = e.target.value;
                    setCidSearch(val);
        ...
        {/* Bloco 4: Procedimentos e Exames */}
        <div className="glass rounded-2xl p-6 space-y-6">
          <h2 className="text-xl font-semibold text-foreground flex items-center gap-2 border-b border-border/50 pb-4">
        ...
        <div className="glass rounded-2xl p-6">
          ...
          <textarea
            ...
            placeholder="Digite notas clínicas ou operacionais adicionais..."
